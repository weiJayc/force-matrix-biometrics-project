...
def run_few_shot_experiment(
    ...
    print(
        "\n測試："
        "\n5 / 10 / 20 筆註冊資料"
        "\n每次 Authentication 使用 30 筆 genuine test"
        "\n每次 Authentication 使用其他使用者各 30 筆 impostor test"
        f"\n每種設定重複 {repeats} 次"
    )
    ...
    for registration_count in registration_counts:
        ...
        for template_user in USERS:
            ...
            for repeat in range(1, repeats + 1):
                ...
                # 和 FORMAL 完全一樣：
                #
                # 每個 user 都重新切 Registration / Test
                # -------------------------------------------------
                ...
                # 和 FORMAL 完全相同
                #
                # 1. Registration-only normalization
                # 2. Template = normalized registration mean
                # 3. Threshold = mean + 2 * std
                # -------------------------------------------------
                ...
                # -------------------------------------------------
                # build_template 本身已經算過 normalization
                #
                # 這裡再次取得 registration-only
                ...
                genuine_rate = float(
                    genuine_rows[0]["acceptance_rate"]
                )
...
def main() -> None:
    ...
    print(
        ...
        "\n  Registration : 5 / 10 / 20 samples"
        "\n  Authentication : 30 unseen samples"
        "\n  Repeated splits : 20"
        "\n  Normalization : Registration-only"
        "\n  Template : Mean of normalized registration samples"
        "\n  Threshold : Mean + 2 * Std of registration distances"
        "\n  Distance : Sequence-level Euclidean"
        "\n  Users : amber / jay / 666 / background"
    )
